        ...
        FastAutoGenerator.create(config.getUrl(), config.getUsername(), config.getPassword())
                ...
                .globalConfig(builder -> {
                    builder.author(config.getAuthor())
                            .outputDir(config.getProjectPath() + "/gpms-server/src/main/java")
                            .disableOpenDir();
                })
                //包名配置
                .packageConfig(builder -> {
                    Map<OutputFile, String> map = new HashMap<>();
                    map.put(OutputFile.xml, config.getProjectPath() + "/gpms-server/src/main/resources/mapper/" + config.getPackageName());
                    builder.parent("com.yongoe.gpms").pathInfo(map).moduleName(config.getPackageName());
                })
                //策略配置
                ...
                .injectionConfig(builder -> {
                    ...
                    String path = config.getProjectPath() + "/gpms-server/src/main/java/com/yongoe/gpms/" + config.getPackageName();
                    builder.customMap(value)
                            ...
                            .customFile(new CustomFile.Builder().fileName(".sql")
                                    ...
                                    .filePath(config.getProjectPath() + "/gpms-ui/src/views")
